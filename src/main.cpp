...
/* 屏幕超时设置 (单位: ms) */
#define SCREEN_ALMOST_TIMEOUT 5000  // 10秒无操作进入低亮度模式
...
void IRAM_ATTR KEY_Detect() {
    ...
    if (timer.enabled) {
        timerCnt++;
        if (timerCnt >= 100) { // 10ms * 100 = 1000ms = 1s
            uint32_t now = millis();
            // 检查是否到达目标时间
...
void setup() {
    ...
    OLED_Init(7, 6, 32, 0); // I2C OLED 初始化: SDA, SCL, 地址等
...
void OLED_ChkTimeout() {
    ...
    if (active) {
        if (!BAT_IS_LOW) { OLED_LowBrightness(false); } // 恢复高亮度
...
void OLED_Update() {
    ...
    if (!screenOn) { return; } // 屏幕关闭时不执行渲染，节省 I2C 总线资源
